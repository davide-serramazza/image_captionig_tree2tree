...
class NIC_Decoder(tf.keras.Model):
    ...
    def call(self, targets, images_emb,pos_embs):
    ...
    def sampling(self,features,pos_embs):
        ...
        for i in range(max_length):
            if i==0:
                ...
                current_word_embs= self.embedding(tf.argmax(predictions, axis=-1))
            ...
            predictions=self.fc(rnn_output)
            to_return=update_matrix(predictions,to_return,ax=1)
